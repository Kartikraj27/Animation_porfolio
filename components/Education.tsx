...
export default function EducationSection() {
  return (
    <section id="education" style={{ minHeight: '80vh', background: '#060606', padding: '100px 40px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
      {/* Subtitle matching your projects style */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 24, maxWidth: 1100, margin: '0 auto' }}>
        {EDUCATION.map(edu => (
          <div key={edu.title} style={{
            ...
            {/* Top row showing Academic Year & Branch Tag */}
            <div style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 14 }}>
              <p style={{ fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(255,140,66,0.7)' }}>
                {edu.tag}
              </p>
              <span style={{ fontSize: 10, fontFamily: 'monospace', color: 'rgba(255,255,255,0.3)', marginLeft: 'auto' }}>
                {edu.academicYear}
              </span>
            </div>

            {/* Degree Title with Bebas Neue font */}
            <h3 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '2.2rem', color: '#fff', marginBottom: 16, tracking: '0.02em' }}>
              {edu.title}
            </h3>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18, padding: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.02)' }}>
              <div style={{ relative: 'true', width: 44, height: 44, flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Image 
                  src={edu.logoUrl} 
                  alt="College Logo" 
                  width={44} 
              ...
              <h4 style={{ fontSize: 14, fontWeight: 500, color: 'rgba(255,255,255,0.85)', lineHeight: 1.4 }}>
                {edu.institution}
              </h4>
            </div>
